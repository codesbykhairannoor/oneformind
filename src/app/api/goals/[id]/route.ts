...
async function proxyToGo(req: Request, userId: string, method: string, id: string, body?: any) {
  ...
  const goUrl = `${protocol}://${host}/api?route=goals${search ? '&' + search.slice(1) : ''}&userId=${userId}&id=${id}`;
...
export async function GET(req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    const data = await proxyToGo(req, session.user.id, 'GET', params.id);
    return NextResponse.json(data);
  } catch (error) {
    console.error('Error proxying GET goal:', error);
...
export async function PUT(req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    console.error('Error proxying PUT goal:', error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
...
export async function DELETE(req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    console.error('Error proxying DELETE goal:', error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
