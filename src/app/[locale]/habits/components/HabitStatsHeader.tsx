        ...
        <div className="relative z-50 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 transition-all duration-500">
            <div className="w-full px-3 sm:px-6 md:px-8 py-3 sm:py-4">
                <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 md:gap-4">
                    ...
                    <div className="flex flex-wrap items-center justify-between md:justify-start gap-2.5 sm:gap-3">
                        <div className="flex items-center gap-2 sm:pr-4 sm:border-r border-slate-100 dark:border-slate-800">
                            <p className="text-xs sm:text-[13px] font-black tracking-wide text-slate-800 dark:text-slate-200">
                                Habit Tracker
                            </p>
                            <span className="px-1.5 py-0.5 rounded-full text-[8px] sm:text-[9px] font-black bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 border border-indigo-100/50">
                                Pro OS
                            </span>
                        </div>

                        {/* Mobile Today Progress Badge (Always visible on mobile!) */}
                        <div className="flex lg:hidden items-center gap-1 px-2 py-0.5 rounded-xl bg-indigo-50/80 dark:bg-indigo-950/40 border border-indigo-100 dark:border-indigo-800/40 text-[10px] font-black text-indigo-600 dark:text-indigo-400">
                            <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse"></span>
                            <span>{todayProgress}%</span>
                            <span className="text-[8px] font-bold text-slate-400 opacity-80">{isIndo ? 'Hari Ini' : 'Today'}</span>
                        </div>

                        {/* Filter Pills (Horizontal scroll on mobile, wrap on desktop) */}
                        <div className="w-full md:w-auto flex items-center gap-1 sm:gap-1.5 overflow-x-auto no-scrollbar py-0.5 order-3 md:order-none">
                            <button
                                type="button"
                                onClick={() => setActiveFilter('all')}
                                className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-xl text-[11px] sm:text-xs font-black shrink-0 transition-all ${
                                    activeFilter === 'all'
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-1.5 sm:gap-2.5 shrink-0">
                        
                        {/* Numeric Mode Switcher (Angka vs Persentase) */}
                        <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-0.5 sm:p-1 rounded-xl border border-slate-200/60 dark:border-slate-700/60 text-[11px] sm:text-xs font-black shadow-xs">
                            <button
                                type="button"
                                onClick={() => onToggleNumericViewMode('value')}
                                title={isIndo ? 'Tampilkan Nilai / Angka' : 'Show Numeric Values'}
                                className={`px-2 sm:px-2.5 py-1 rounded-lg flex items-center gap-1 transition-all ${
                            ...
                            >
                                <span>🔢</span>
                                <span className="hidden xs:inline">{isIndo ? 'Angka' : 'Values'}</span>
                            </button>
                            <button
                                type="button"
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="flex items-center gap-1.5 sm:gap-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-2.5 sm:pl-3 sm:pr-2.5 py-1.5 sm:py-2 rounded-xl font-bold text-slate-700 dark:text-slate-300 hover:border-indigo-300 transition-all text-xs"
                            >
                                <div className="flex flex-col text-left leading-none">
                                    <span className="text-[8px] sm:text-[9px] text-slate-400">{isIndo ? 'Periode' : 'Period'}</span>
                        ...
                        <button
                            ...
                            className="px-3 sm:px-4 py-2 sm:py-2.5 flex items-center gap-1.5 sm:gap-2 text-white rounded-xl font-bold bg-indigo-600 hover:bg-indigo-700 shadow-md shadow-indigo-100 dark:shadow-none transition-all active:scale-95 text-xs"
                        >
                            <Plus size={15} strokeWidth={3} />
                            <span className="font-black">{t('habits_add_btn') || 'Tambah'}</span>
                        </button>
                    </div>
