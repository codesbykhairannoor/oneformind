...
export default function Register() {
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        if (!name.trim()) {
            newErrors.name = t('auth_val_name') || 'Nama lengkap wajib diisi';
            hasError = true;
        }
        ...
        if (!password || password.length < 6) {
            newErrors.password = t('auth_val_pass_min') || 'Password minimal 6 karakter';
            hasError = true;
        }
        ...
        <div className="relative min-h-screen flex flex-col items-center justify-center bg-white selection:bg-indigo-100 selection:text-indigo-700 p-6 overflow-hidden">
            ...
            <div className="w-full max-w-[420px] flex flex-col relative z-10">
                ...
                <form onSubmit={submit} className="space-y-4">
                    <div>
                        <input
                            ...
                            className={`w-full px-4 py-3 text-sm bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-4 transition-all outline-none ${
                                errors.name ? 'border-red-400 focus:ring-red-100 focus:border-red-500' : 'focus:border-indigo-500 focus:ring-indigo-500/10'
                            }`}
                        />
                        <InputError className="mt-1.5 ml-1" message={errors.name} />
                    </div>
                    ...
                    >
                        <span>{isProcessing ? 'Daftar...' : (t('auth_btn_register') || 'Daftar Sekarang')}</span>
                        {!isProcessing && (
