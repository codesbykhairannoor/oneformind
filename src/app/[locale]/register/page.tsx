...
export default function Register() {
    ...
    const handleEmailRegister = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (password !== confirmPassword) {
            setState({ success: false, error: 'Password tidak cocok' });
            setIsPending(false);
            return;
        }
            ...
            <div className="w-full max-w-[420px] flex flex-col relative z-10">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-sm font-medium text-slate-500">
                        {t('auth_has_account') || 'Sudah punya akun?'}{' '}
                ...
                <button
                    ...
                    disabled={isPending}
                    className="flex items-center justify-center w-full px-4 py-3 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm dark:shadow-none bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-slate-200 mb-6 group active:scale-[0.98] disabled:opacity-75 disabled:cursor-not-allowed">
                    {isPending ? (
                        <span className="font-bold text-slate-700 text-sm">Memproses...</span>
                    ) : (
                        <>
                            ...
                            <span className="font-bold text-slate-700 text-sm">{t('auth_btn_google_up') || 'Lanjutkan dengan Google'}</span>
                        </>
                    )}
                </button>
                ...
                <form onSubmit={handleEmailRegister} className="space-y-4">
                    <div>
                        <input
                            ...
                            name="name"
                            type="text"
                            placeholder={t('auth_placeholder_name') || 'Nama Lengkap'}
                    ...
                    <div className="relative">
                        <input
                            ...
                            placeholder={t('auth_placeholder_pass') || 'Password (min. 6 karakter)'}
                            className="w-full pl-4 pr-10 py-3 text-sm bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-4 transition-all outline-none focus:border-indigo-500 focus:ring-indigo-500/10"
                            minLength={6}
                            required
                        />
                    ...
                    <button
                        type="submit"
                        disabled={isPending}
                        className="w-full mt-4 bg-indigo-600 text-white font-black py-3.5 rounded-xl shadow-lg shadow-indigo-100 dark:shadow-none hover:bg-indigo-700 transition transform hover:-translate-y-0.5 active:scale-95 disabled:opacity-75 flex items-center justify-center gap-2 cursor-pointer"
                    >
                        <span>{isPending ? 'Memproses...' : (t('auth_btn_register') || 'Daftar Sekarang')}</span>
                        {!isPending && (
                    ...
                    <p className="text-[10px] text-slate-400 text-center leading-relaxed mt-4">
                        {t('auth_consent_notice') || 'Dengan mendaftar, Anda menyetujui Ketentuan Layanan dan Kebijakan Privasi kami.'}
                    </p>
                </form>
